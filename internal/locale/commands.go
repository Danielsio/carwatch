...
var heCommands = map[string]string{
	...
	// stale callback
	"callback_expired": "⏳ הכפתור הזה כבר לא פעיל. השתמש ב /watch כדי להתחיל חיפוש חדש.",
	...
	"fmt_hand_inline":            "✋ יד %d",
	"fmt_location_inline":        "📍 %s",
	"fmt_price":                  "💰 מחיר: ₪%s\n",
	"fmt_price_drop":             "💰 *ירידת מחיר!* %s: ₪%s → ₪%s (-₪%s)\n",
	// formatter — at-a-glance verdict + compact spec tokens
	"fmt_fitness_inline":                "🎯 התאמה: %.1f/10",
	"fmt_deal_inline":                   "📊 עסקה: %d/100",
	"fmt_mileage_inline":                "🛣️ %s ק\"מ",
	"fmt_engine_inline":                 "⚙️ %.1fL",
	"fmt_fuel_inline":                   "⛽ %s",
	"fmt_power_inline":                  "🐴 %d כ\"ס",
	"fmt_seller_private":                "👤 מוכר פרטי",
	"fmt_seller_dealer":                 "🏢 סוחר",
	"fmt_suspicious_header":             "⚠️ *מודעה חשודה*\n",
	"fmt_suspicious_price_below_market": "• המחיר נמוך משמעותית ממחיר השוק — מומלץ לבדוק\n",
	"fmt_suspicious_no_photo_low_price": "• אין תמונה והמחיר נמוך מהרגיל\n",
...
var enCommands = map[string]string{
	...
	// stale callback
	"callback_expired": "⏳ This button has expired. Use /watch to start a new search.",
	...
	"fmt_hand_inline":            "✋ Hand %d",
	"fmt_location_inline":        "📍 %s",
	"fmt_price":                  "💰 Price: ₪%s\n",
	"fmt_price_drop":             "💰 *Price Drop!* %s: ₪%s → ₪%s (-₪%s)\n",
	// formatter — at-a-glance verdict + compact spec tokens
	"fmt_fitness_inline":                "🎯 Fitness: %.1f/10",
	"fmt_deal_inline":                   "📊 Deal: %d/100",
	"fmt_mileage_inline":                "🛣️ %s km",
	"fmt_engine_inline":                 "⚙️ %.1fL",
	"fmt_fuel_inline":                   "⛽ %s",
	"fmt_power_inline":                  "🐴 %d HP",
	"fmt_seller_private":                "👤 Private seller",
	"fmt_seller_dealer":                 "🏢 Dealer",
	"fmt_suspicious_header":             "⚠️ *Suspicious listing*\n",
	"fmt_suspicious_price_below_market": "• Price far below market — verify carefully\n",
	"fmt_suspicious_no_photo_low_price": "• No photo and an unusually low price\n",
