...
func migrateUserDigestColumns(db *sql.DB) error {
...
func migrateListingHistoryChatID(db *sql.DB) error {
...
func migrateUserLanguage(db *sql.DB) error {
...
func migrateSearchKeywords(db *sql.DB) error {
...
func migrateSearchUserSeq(db *sql.DB) error {
...
func migrateUserTierColumns(db *sql.DB) error {
...
func migrateUserDailyDigest(db *sql.DB) error {
...
func migrateListingPerfIndexes(db *sql.DB) error {
	if _, err := db.Exec(`
...
func migrateDropLegacyCacheTables(db *sql.DB) error {
...
func migrateUserChannelColumns(db *sql.DB) error {
	for _, col := range []struct {
		name string
		def  string
	}{
...
func migrateChannelIDIndexAndBackfill(db *sql.DB) error {
	if _, err := db.Exec(`
...
func migrateSearchShareToken(db *sql.DB) error {
...
func migratePriceHistoryV2(db *sql.DB) error {
...
func migrateListingHistoryFitnessScore(db *sql.DB) error {
...
func migrateListingHistoryImageURL(db *sql.DB) error {
...
func migrateUserLastSeenAt(db *sql.DB) error {
...
func migrateListingHistorySearchID(db *sql.DB) error {
...
func migrateLinkTokensTable(db *sql.DB) error {
	if _, err := db.Exec(`
...
func migrateUserLinkedWebID(db *sql.DB) error {
...
func migrateUserLinkedWebIndex(db *sql.DB) error {
	if _, err := db.Exec(`
...
func migrateUsersActiveIndex(db *sql.DB) error {
	if _, err := db.Exec(`
...
func migrateSearchPriceMinAndGearBox(db *sql.DB) error {
	for _, col := range []struct {
		name string
		def  string
	}{
